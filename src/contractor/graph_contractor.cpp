...
void RenumberGraph(std::vector<RemainingNodeData> &remaining_nodes,
                   ...
                   ContractorGraph &graph)
{
    ...
    graph.Renumber(current_to_new_node_id);
    ...
    for (const auto node : util::irange<NodeID>(0, graph.GetNumberOfNodes()))
    {
        for (const auto edge : graph.GetAdjacentEdgeRange(node))
        {
            ...
            if (data.shortcut)
            {
                data.id = current_to_new_node_id[data.id];
            }
        }
    }
}
                           ...
                           double core_factor)
{
           ...
           number_of_contracted_nodes < static_cast<NodeID>(number_of_nodes * core_factor))
    {
        ...
        {
            RenumberGraph(remaining_nodes, new_to_old_node_id, node_data, graph);
            log << "[renumbered]";
    ...
    graph.Renumber(new_to_old_node_id);
